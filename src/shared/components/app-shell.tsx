...
export function AppShell({ children }: PropsWithChildren) {
  return (
    <div className="min-h-dvh bg-[#f4f7f5] text-slate-950">
      <header className="sticky top-0 z-40 border-b border-slate-900/6 bg-[#f4f7f5]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[72px] max-w-[1600px] items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button type="button" className="user-summary" aria-label="Abrir menu da conta">
              <span className="flex size-9 items-center justify-center rounded-full bg-[#d9eee7] text-sm font-semibold text-[#155f4f]">
                LF
              </span>
              <span className="hidden text-left sm:block">
                <span className="block text-sm font-semibold leading-tight text-slate-900">Luis Fernando</span>
                <span className="mt-0.5 block text-xs leading-tight text-slate-500">Conta individual</span>
              </span>
            </button>
          </div>
        </div>
      ...
      <div className="mx-auto flex max-w-[1600px]">
        <aside className="sticky top-[72px] hidden h-[calc(100dvh-72px)] w-[256px] shrink-0 flex-col px-6 py-8 lg:flex">
          ...
          <div className="mt-auto rounded-2xl bg-[#e7f1ed] p-4 shadow-[inset_0_0_0_1px_rgba(15,107,87,0.08)]">
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#287765]">Precisa de ajuda?</p>
            <p className="mt-2 text-sm leading-5 text-slate-600">Fale com seu assessor pelo canal seguro da plataforma.</p>
            <Link to="/services" className="mt-3 inline-flex min-h-10 items-center text-sm font-semibold text-[#0f6b57]">
              Acessar atendimento
            </Link>
          </div>
        </aside>
